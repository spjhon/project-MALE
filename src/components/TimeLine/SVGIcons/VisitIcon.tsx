const VisitIcon = () => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="800px"
      height="800px"
      viewBox="0 -0.5 17 17"
      version="1.1"
      className="si-glyph si-glyph-retroeexcavadora"
    >
      <title>694</title>

      <defs></defs>

      <g stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
        <g transform="translate(1.000000, 1.000000)" fill="#fff">
          <circle
            cx="12.473"
            cy="11.473"
            r="1.473"
            className="si-glyph-fill"
          ></circle>

          <path
            d="M3.502,6.035 C1.566,6.035 0,7.588 0,9.503 C0,11.418 1.566,12.97 3.502,12.97 C5.434,12.97 7.002,11.418 7.002,9.503 C7.002,7.588 5.434,6.035 3.502,6.035 L3.502,6.035 Z M3.502,11.162 C2.576,11.162 1.825,10.42 1.825,9.503 C1.825,8.586 2.576,7.843 3.502,7.843 C4.426,7.843 5.177,8.586 5.177,9.503 C5.177,10.42 4.426,11.162 3.502,11.162 L3.502,11.162 Z"
            className="si-glyph-fill"
          ></path>

          <path
            d="M15.332,6.68 C15.332,6.186 14.934,5.784 14.441,5.784 L13.937,5.714 L13.937,4.982 L13.961,4.982 L13.961,2.121 L13.001,2.121 L13.001,4 L12.03,4 L12.03,5.451 L9.594,5.115 C8.992,3.447 7.565,0.094 7.006,0.094 L1.556,0.763 C0.563,0.763 0.03,6 0.03,6 C0.702,5.611 2.572,4.938 3.405,4.938 C5.931,4.938 8.046,7.098 8.046,9.626 C8.046,10.105 7.967,10.528 7.83,10.964 L9.931,10.964 C10.077,9.719 11.038,8.835 12.324,8.835 C13.607,8.835 14.654,9.725 14.801,10.97 L15.11,10.97 C15.602,10.97 16.001,10.649 16.001,10.154 L15.332,6.68 L15.332,6.68 Z M2.938,4.031 L2.938,1.937 L6.063,1.437 L7.063,4.031 L2.938,4.031 L2.938,4.031 Z"
            className="si-glyph-fill"
          ></path>
        </g>
      </g>
    </svg>
  );
};

export default VisitIcon;
